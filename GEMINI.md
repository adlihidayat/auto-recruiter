...
│ └── realtime-worker/ # Python LiveKit Agents SDK worker
├── packages/
│ ├── core-ai-lib/ # Shared Python code (Schemas, tracing, clients for backend/agents/worker)
│ └── shared-ui/ # Shared React/TS code (UI components for frontend)
├── .env # Global Environment Variables
├── pyproject.toml # Root UV workspace configuration
└── package.json # Root NPM workspace configuration
```

### Global Workspace Rules (No Local Files)

To maintain a clean and centralized monorepo, **you must never create localized environments or configuration files inside individual applications**.
- **No Local `.env`**: All secrets and configuration must be placed in the root `/.env` file. Do not create `.env` or `.env.local` inside `apps/*`. Python scripts should point to `ROOT_DIR/.env` via `load_dotenv()`, and `package.json` scripts should inject it using `dotenv -e ../../.env`.
- **No Local `node_modules`**: All npm dependencies are hoisted to the root. Do not manually run `npm install` inside an app directory unless utilizing the root workspace commands.
- **No Local `.venv`**: Python dependencies are managed entirely via the root `uv` workspace. Never run `uv venv` inside individual `apps/*`.
...
- **Shared packages**: `apps/backend`, `apps/agents`, and `apps/realtime-worker` are all Python and must import shared payload shapes (`Goal`, `QuestionSuite`, `InterviewSession`, etc.) from `packages/core-ai-lib` rather than redefining local copies. `apps/frontend` must import all common React components from `packages/shared-ui`.
