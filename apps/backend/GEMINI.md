...
5. **`weight` and `communication_weight` are set here, not by any agent.** Per the grader agent's own boundary doc, these are a human judgment call. If the create-interview form doesn't yet collect them, default every goal to `weight: 1` and the plan to `communication_weight: "low"` — but treat this as a placeholder to build real UI for, not a permanent decision.
