...
PLANNER_EVAL_SYSTEM_INSTRUCTION = """
Revised PLANNER_EVAL_SYSTEM_INSTRUCTION.

WHAT CHANGED FROM YOUR VERSION, AND WHY:
Only Dimension 3 (edge-case compliance) was touched. Relevance and Coverage rubric wording is
untouched deliberately — those are tuning nuances, not correctness bugs.

The bug: `contradiction_flagged` and `discriminatory_content_excluded` are defined as checks on
"metadata warnings," but nothing in the prior evidence made clear that this must come from an
explicit warnings/assumptions field on the CANDIDATE output — not from the judge's own inference
about whether the plan's goals happen to reflect the JD's problematic content. On scenario 4b,
this produced a false positive: the judge saw goals that clearly mirrored the JD's contradictory
(lead-level) asks and apparently concluded "the contradiction must have been noticed," when in
fact meta.warnings was empty and the plan never flagged anything — it just uncritically executed
the bad JD content. Reflecting a problem and flagging a problem are opposite behaviors; the
previous prompt didn't force the judge to tell them apart.

ACTION REQUIRED ON YOUR SIDE (not just this prompt):
Your PLANNER_EVAL_USER_TEMPLATE currently does not pass the candidate's `meta` (warnings/
assumptions) into the judge's input at all — only `goals_json`. This prompt fix assumes that
gap is closed. If `meta` still isn't included in what you send the judge, these two checks have
no legitimate evidence to work from regardless of prompt wording, and should be expected to keep
...
======================================================================
GROUNDING RULE (apply before scoring anything)
======================================================================
...
For each check, output `true`, `false`, or `"partial"` based on direct evidence. Output `null`
